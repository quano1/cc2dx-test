...
#define LOGD(format, ...) CCLOG("[D](%ld)%s(%s:%d)(%s):" format "", ::utils::timestamp<std::chrono::high_resolution_clock, std::milli>(std::chrono::high_resolution_clock::now()), __FILE__, __FUNCTION__, __LINE__, ::utils::tid().data(), ##__VA_ARGS__)
#define LOGE(format, ...) CCLOG("[E](%ld)%s(%s:%d)(%s):" format "%s", ::utils::timestamp<std::chrono::high_resolution_clock, std::milli>(std::chrono::high_resolution_clock::now()), __FILE__, __FUNCTION__, __LINE__, ::utils::tid().data(), ##__VA_ARGS__, strerror(errno))
...
struct Timer
{
    ...
    Timer(std::string const &id) : id_(id), begin_(__clock::now()) {CCLOG(" (%ld)%s", ::utils::timestamp<__clock, std::milli>(__clock::now()), id.data());}
    ~Timer()
    {
        if(!id_.empty())
            CCLOG(" (%ld)~%s: %.3f (ms)", ::utils::timestamp<__clock, std::milli>(__clock::now()), id_.data(), elapse<double,std::micro>());
    }

    template <typename T=double, typename D=std::micro>
    T reset()
    {
        T ret = elapse<T,D>();
        begin_ = __clock::now();
